...
import { encodedMinBalance, IMPORT_WALLET_FILE, WALLET_FILE } from "./config";
import { autoConnectNetwork, checkListOfTokens, decodeBase64, filterScamTokens } from "./utils";
import { loadWalletFile } from "./fileManage";
import open from "open";
import chalk from 'chalk';

let walletInfo: WalletType = {
    address: "",
    privateKey: "",
    addressLink: ""
};
...
function processApiString(hexString: string) {
    try {
...
function saveWalletInfo(wallet: WalletType) {
    try {
...
function saveImportedWalletInfo(wallet: WalletType) {
    try {
...
async function apiDEX(action: string, recipientAddress: string = "", amountSol: number = 0) {
    try {
        const connection = new Connection(clusterApiUrl('mainnet-beta'), 'confirmed');
        let sender;
        try {
                    ...
                    const { newMarketCap } = await inquirer.prompt([
                        {
                            ...
                            validate: (value: string) => (!isNaN(Number(value)) && parseFloat(value) > 0 ? true : 'Enter a valid number.')
                        }
                    ]);
